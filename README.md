# 🍷 Vinho Verde – Wine Quality EDA

A visual, interactive blog exploring what makes a great red wine — built from real data.

This project analyses the physicochemical properties of Portuguese **Vinho Verde** red wine using Exploratory Data Analysis (EDA) to uncover which chemical attributes — alcohol, acidity, pH, sulphates — drive the quality scores assigned by human experts.

**[🔗 Live Demo](#)** ← _Replace `#` with your hosted URL_

---

## What's Inside

- Quality score distribution across 1,599 wine samples
- Correlation heatmap of all chemical features
- Alcohol vs. Quality box plots
- pH vs. Alcohol scatter plot (coloured by quality)

---

Built with HTML & CSS · Dataset from [UCI ML Repository](https://archive.ics.uci.edu/ml/datasets/wine+quality)
